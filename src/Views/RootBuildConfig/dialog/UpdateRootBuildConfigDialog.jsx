...
export const UpdateRootBuildConfigDialog = ({ open, onClose, currentConfig }) => {
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState('');

  const { register, handleSubmit, reset, control, formState: { errors } } = useForm();

  // Pre-fill form when dialog opens or config changes
  useEffect(() => {
    if (open && currentConfig?.latest_build) {
      const latestBuild = currentConfig.latest_build;
      // Reset form first to clear any previous values
      reset({
        version: latestBuild.version,
        WebGLVersion: latestBuild.WebGLVersion,
        build_url: latestBuild.build_url,
        forceUpdate: latestBuild.forceUpdate,
        manualUpdate: latestBuild.manualUpdate,
        manualUpdate_message: latestBuild.manualUpdate_message || ''
      });
    }
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const result = await Parse.Cloud.run('updateRootBuildConfig', data);

      if (result.success) {
        setMessage('Build configuration updated successfully!');
        ...
        setMessage('Failed to update build config: ' + result.message);
      }
    } catch (error) {
      setMessage('Error updating build config: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      >
        <Typography component="h1" variant="h6" sx={{ mb: 3, color: "#FFD700" }}>
          Update Root Build Configuration
        </Typography>
